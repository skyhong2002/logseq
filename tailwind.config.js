...
// it will also loop through all 5 color stops, unless the stops are specifed 
// grad-bg-stops-3
// this will have a default repeating gradient at a step that can be configued with 
// grad-bg-cycle-32 
const addGradientColors = plugin(({ addBase, addComponents, addUtilities, config, ___theme }) => {
  ...
  Object.values(gradientColors).forEach((stops, ___index) => {
    ...
    addComponents({
      // tailwind compoent for .grad-bg-COLOR-9
      [`.grad-bg-${baseColor}-9`]: {
      ...
      // tailwind compoent for .grad-bg-COLOR-9-alpha
